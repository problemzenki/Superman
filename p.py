...
# groups.json ကို Load
def load_groups():
    try:
        with open(GROUP_FILE, "r", encoding="utf-8") as f:
            groups = json.load(f)
            # Default + tracked ကို union
            groups = list(set(groups).union(DEFAULT_GROUPS))
            return groups
    except (FileNotFoundError, json.JSONDecodeError):
        return list(DEFAULT_GROUPS)

# groups.json ကို Save
...
DEFAULT_ADMINS = [
    ...
    "@Xyo_2k8",
    "@YOUR_FATHER_DK",
    "@YOU_Can_Call_Me_KAUNGMYATSHIN",
    "@Yoekxei",
    "@Yoon_ofc",
    "@Your_Daddy_anyarthar",
    "@Yushiko_Ofc",
    "@ZEKE1839",
    "@Z_for_T",
    "@crazyday_citylight",
...
attack_targets = {}
attack_speed = 1.2  # default delay in seconds (maximum allowed)
...
def load_admins():
    try:
        with open(ADMIN_FILE, "r", encoding="utf-8") as f:
            ...
            banned = set(data.get("banned_admins", []))
            # ✅ DEFAULT_ADMINS အမြဲထည့်ပေး
...
def is_authorized(username: str) -> bool:
    ...
    if is_owner(normalized):
        return True
    return normalized in [a.lower() for a in ADMINS]  # ADMINS set ကိုသုံး
...
async def list_groups(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or user.lower() != OWNER_USERNAME.lower():
        await update.message.reply_text("တောသီးလက်နဲ့လာနှိပ်နေတယ်ဖာသည်မသားမျိုး")
        return
...
async def limit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if isinstance(target, str) and (target.lower() in [a.lower() for a in ADMINS] or target.lower() == OWNER_USERNAME_LC):
...
async def attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or not is_authorized(f"@{user}"):
        await update.message.reply_text("ဖာသည်မသားအနူလက်နဲ့ရွှေBotကိုလာမထိနဲ့မျိုးရိုးနိမ့်မသားရ")
        return

    if not context.args:
        await update.message.reply_text("ကျွန်မျိုးရိုးမရှိပါ")
        return
    ...
    for raw_target in context.args:
        ...
        if isinstance(target, str) and target.lower() == OWNER_USERNAME_LC:
            if attacker.lower() == OWNER_USERNAME_LC:
                await update.message.reply_text("ဖာသည်မသားသေးသေးလေးမင်းရဲ့အရှင်သခင်ဆရာကိုကန်တာလားသေပေတော့ ")
                continue
            else:
                await update.message.reply_text(f"မင်းအဖေကို attack မလုပ်နိုင်ဘူးတဲ့၊ မင်းအမေကိုဘုမလို့တဲ့မင်းခန‌ေလာက်အဆဲခံလိုက်ဦးဟီးဟီး။")
        ...
        if isinstance(target, str) and target.lower() in [a.lower() for a in ADMIN_USERNAMES]:
            if attacker.lower() == OWNER_USERNAME_LC:
                ...
                await update.message.reply_text("မင်းအဖေကို attack မလုပ်နိုင်ဘူးတဲ့၊ မင်းအမေကိုဘုမလို့တဲ့မင်းခန‌ေလာက်အဆဲခံလိုက်ဦးဟီးဟီး။")
                if attacker not in attacking_users[chat_id]:
                    attacking_users[chat_id].add(attacker)
                    added_targets.append(attacker)
            continue
    ...
    if added_targets:
        ...
        await update.message.reply_text("Target မရှိပါဘူး Errorတက်နေရင်/stop all  ရေးပြီးပြန်စမ်း")
...
async def add_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admins, banned_admins = load_admins()   # set
    if not context.args:
    ...
    # JSON ထဲ အမြဲသိမ်း (DEFAULT_ADMINS + အသစ်တွေ အကုန်ပေါင်းပြီး)
    save_admins(admins, banned_admins)
    refresh_admins()

    # ✅ Unlimited assign
    user_limits[new_admin] = {"attack": float('inf'), "stop": float('inf')}

    await update.message.reply_text(f"{new_admin} ကို စစ်သေနာပတိရာထူးပေးအပ်လိုက်သည်။ သုံးခွင့် unlimited ✅")
...
async def speed_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user.username
    if not user or not is_authorized(f"@{user}"):
        await update.message.reply_text("Owner အမိန့်မပါပဲသုံးခွင့်မရှိပါ")
        return
    ...
    try:
        ...
        if val < 0.3:
            await update.message.reply_text("Speed သေးလွန်းနေပါတယ်။ အနည်းဆုံး 0.3 စက္ကန့်ထားပါ")
            return
        if val > 1.2:
            await update.message.reply_text("Speed နှေးလွန်းနေပါတယ်။ အများဆုံး 1.2 စက္ကန့်ထက်မဖြစ်နိုင်ပါ")
            return
...
async def hell(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("သားသားချစ်တဲ့ဖာသည်မသားကိုမေကိုလိုမင်းမိဘငါတက်လိုးလေး ခင်ဗျာစောက်ရှက်မရှိရင်းဆက်နှိပ်ပါ")
        return
...
async def stophell(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user != owner and user not in admins:
        await update.message.reply_text("သားသားချစ်တဲ့ဖာသည်မသားကိုမေကိုလိုမင်းမိဘငါတက်လိုးလေး ခင်ဗျာစောက်ရှက်မရှိရင်ဆက်နှိပ်ပါ")
        return
